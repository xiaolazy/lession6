    ...
    <li class="shuttle_box_li shuttle_box_near">
        <ul id="shuttle_box_left">
            <c:forEach items="${subjects}" var="subs">
                <li value="${subs.id}">${subs.name}</li>
            </c:forEach>
        </ul>
    </li>
    <li class="shuttle_box_li" id="shuttle_box_mid">
        <input type="button" id="shuttle_box_toRight" value="选择此课程"/>
        <input type="button" id="shuttle_box_toLeft" value="移除此课程"/>
    </li>
    <li class="shuttle_box_li shuttle_box_near">
        <ul id="shuttle_box_right">
            <c:forEach items="${data.subjects}" var="alerd">
                <li value="${alerd.id}">${alerd.name}</li>
            </c:forEach>
        </ul>
    </li>
</ul>
...
<script type="text/javascript">
    ...
    function submit() {
        ...
        var dataFormJson={'id':id,'ids':JSON.stringify(temp)};
        operate.add(dataFormJson,'/grade/save_edit.do?id='+id,"选课")
    }
    //穿梭框左侧选中
